...
//  TIPE DATA LOP
// ──────────────────────────────────────────────────────────────
export interface ExistingLOPData {
  idIhld: string;
  lop: string;
  sto: string;
  regionTif: string;
...
//  PROPS
// ──────────────────────────────────────────────────────────────
interface AddLopMitraAdminProps {
  isOpen: boolean;
  onClose: () => void;
  existingData?: ExistingLOPData | null;
}
...
export default function AddLopMitraAdmin({ isOpen, onClose, existingData }: AddLopMitraAdminProps) {
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-auto relative flex flex-col" style={{ height:"500px" }}>
            ...
            <div className="pt-5 px-8 pb-3 flex-shrink-0">
              <h2 className="text-xl font-bold text-gray-800 text-center">
                {isIsiWaspangMode ? "Isi Waspang Mitra" : "Tambahkan LOP Baru"}
              </h2>
              {isIsiWaspangMode && (
                <div className="mt-2 flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  ...
                  <p className="text-xs text-amber-700">
                    Data LOP sudah terisi. Lengkapi <span className="font-semibold">Waspang Mitra</span> di bawah ini.
                  </p>
                </div>
              )}
            </div>
                    ...
                    {currentStep === 1 && (
                      <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                        ...
                        <div className={isIsiWaspangMode ? "col-span-2 bg-amber-50 border border-amber-200 rounded-lg p-3" : ""}>
                          {isIsiWaspangMode && (
                            <p className="text-xs font-semibold text-amber-700 mb-2">Isi Waspang Mitra di sini</p>
                          )}
                          <Label required>Waspang Mitra</Label>
                          <input name="waspangMitra" value={form.waspangMitra} onChange={handleChange}
                            placeholder="Nama waspang mitra..."
